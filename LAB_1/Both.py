...
#  (y' - y)**2  / N;
forward_pass_loss = np.sum((predicted_output - target_output) ** 2) / input_data.shape[0]

print("Forward Pass Loss:", forward_pass_loss)

#  dl/dw

dL_dw = 2* np.dot(input_data.T, (predicted_output - target_output))

#  dl_db
dL_db = 2* np.sum(predicted_output - target_output) / input_data.shape[0]

h = 0.001
weights[0] = + h # reduce a Miner(0.001) value from each weight 

predicted_output = np.dot(input_data, weights) + bias
#  (y' - y)**2  / N;
new_forward_pass_loss = np.sum((predicted_output - target_output) ** 2) / input_data.shape[0]
...
backward_pass_loss =  np.sum((predicted_output) ** 2) / input_data.shape[0]
...
print("New NGradient of bias:", (dL_db -New_dL_db) / h)
